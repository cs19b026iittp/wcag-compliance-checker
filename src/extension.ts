		...
		async provideHover(document, position, token) {
			...
			console.log(word);
				...
				if (word == "input") {
					...
					const s4 = '<h4>anchor tag</h4>'
					markdown.appendMarkdown(s4);
					if(desc==1){  // if anchor tag does not have description
						markdown.appendMarkdown("Enter description");
					}
					if(aria_lab==1){  // if id doesnt match aria-label
						markdown.appendMarkdown("<p>Guideline 2.5.3 Label in Name</p>");
						markdown.appendMarkdown("<p>each ID in the value of the aria-labelledby attribute matches an ID of a text element used as part of the link purpose.</p>");
					}
					markdown.supportHtml=true;
					markdown.isTrusted=true;
					if(! decorated.includes(i)){
				...
				if (src_code[i].includes(keywords[j])) {
					...
					if (j == 1) {
						...
						if (match_keyword != null && match_keyword.index !== undefined) {
							...
							var id_name = "";
							var f=0;
							if(src_code[i].includes("id=")){  // to get the id
								var id_ind = src_code[i].indexOf("id=")
								for(var k1 = id_ind+3;k1<src_code[i].length;k1++){
									if(src_code[i].charAt(k1)=='"'){
										for(var k2 = k1+1;k2<src_code[i].length;k2++){
											if(src_code[i].charAt(k2)=='"'){
												// getting the id
												id_name=src_code[i].substring(k1+1,k2);
												f=1;
												break;
											}
										}
							...
							console.log(id_name);
							f=0;
							var aria_labelled_name="";
							if(src_code[i].includes("aria-labelledby")){  // to get the aria-labelled by attribute
								var id_ind = src_code[i].indexOf("aria-labelledby")
								for(var k1 = id_ind+3;k1<src_code[i].length;k1++){
									if(src_code[i].charAt(k1)=='"'){
										for(var k2 = k1+1;k2<src_code[i].length;k2++){
											if(src_code[i].charAt(k2)=='"'){
												// getting the id
												aria_labelled_name=src_code[i].substring(k1+1,k2);
												f=1;
												break;
											}
										}
							...
							var count=0;
							for(var i1=0;i1<aria_contents.length;i1++){
								if(id_name.includes(aria_contents[i1])){
									count++;
								}
							}
							if(count==0){
