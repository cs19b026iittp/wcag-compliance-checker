...
function hover_button(x:any):void{   // displaying the content on hover
vscode.languages.registerHoverProvider('html', {
    async provideHover(document, position, token) {
        ...
        if(word == "button" && x==8){
            markdown.appendMarkdown("button");
		    markdown.supportHtml=true;
		    markdown.isTrusted=true;
		    return	new vscode.Hover(markdown,new vscode.Range(position,position));
        }
        else if(word == "input" && x==0){
   ...
   export async function  css_file_cr(model : {p1:string;p2:boolean,p3:any,p4:any,p5:any,p6:any}){
